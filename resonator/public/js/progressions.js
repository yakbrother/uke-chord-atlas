...
function renderCardBack(progression) {
  ...
  for (let i = 0; i < progression.chords.length; i++) {
    ...
    // Get drone (primary/fingerstyle) and full (barre/slide) voicings for this root
    const droneVoicings = getVoicings(rootIndex, { mode: 'drone', includeString1: true });
    const fullVoicings = getVoicings(rootIndex, { mode: 'full' });
    const matchesType = v => chordType && (v.type === chordType || v.label === chordType);

    // Priority: drone-match -> drone-clash -> full, preferring the specific
    // chord type where possible, only falling back to full 6-string/barre
    // shapes when no fingerstyle voicing exists at all for this chord.
    const selectedVoicing =
      droneVoicings.find(v => v.droneFit === 'match' && matchesType(v)) ||
      droneVoicings.find(v => v.droneFit === 'match') ||
      droneVoicings.find(v => v.droneFit === 'clash' && matchesType(v)) ||
      droneVoicings.find(v => v.droneFit === 'clash') ||
      fullVoicings.find(matchesType) ||
      fullVoicings[0] ||
      null;
    ...
    if (selectedVoicing) {
      ...
      const noteNamesArr = selectedVoicing.notes.map(n => (n === null ? '' : noteNames[n % 12]));
      ...
      const svg = chordSVG(
        ...
        noteNamesArr,
        rootIndex,
        selectedVoicing.frets.some(f => f === 0) && selectedVoicing.frets.some(f => typeof f === 'number' && f > 0),
        { droneFit: selectedVoicing.droneFit }
      );
